/*
   OpenPciInfo v1.6 PCI DEBUG coded by Titan Email : bvernoux@wanadoo.fr
   03/06/2006
   - Special version to display 8/16/32bit Hex-Dump device configuration space
   01/06/2006
   - Special version to display Hex-Dump device configuration space
   12/03/2005
...
int main(int argc, char **argv)
{
 ...
 {                    
    ...
    {
        printf("\n----------------------------------\n");
        printf("8bit Hex-Dump device configuration space\n");

        for(i=0;i<256;i+=16)
        {
            printf(" %04lx ",(long)i);    // Address
            for(j=0;j<16;j++)
            {
                config=(long)pci_read_config_byte(i+j,dev);
                config&=0xff;
                printf(" %02lx",config);    // Value
            }
            printf("\n");
        }

        printf("\n----------------------------------\n");
        printf("16bit Hex-Dump device configuration space\n");
        ...
        for(i=0;i<256;i+=16)
        {
            ...
            for(j=0;j<16;j+=2)
            {
                config=(long)pci_read_config_word(i+j,dev);
                config&=0xffff;
                printf(" %04lx",config);    // Value
            }
            printf("\n");
        }

        printf("\n----------------------------------\n");
        printf("32bit Hex-Dump device configuration space\n");
        ...
        for(i=0;i<256;i+=16)
        {
            ...
            for(j=0;j<16;j+=4)
            {
                config=(long)pci_read_config_long(i+j,dev);
                printf(" %08lx",config);    // Value
            }
            printf("\n");
        }
